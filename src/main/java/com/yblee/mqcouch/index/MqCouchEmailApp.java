...
//			System.out.println("root dir path : " + rootPath);
    ...
    private static void checkArgsForOption(String[] args) {
    	...
    	if(args.length == 2) {
    		if(args[0].compareTo("-encrypt") == 0 || args[0].compareTo("--encrypt") == 0) {
        		...
        		System.exit(0);
    		} else if(args[0].compareTo("-decrypt") == 0 || args[0].compareTo("--decrypt") == 0) {
    			// will take second argument and give you encriped password
        		String rawPass = args[1].trim();
        		String encryptedPass = "";
        		try {
        			encryptedPass = SimpleProtector.decrypt(rawPass);
        		} catch(Exception e) {
        			e.printStackTrace();
        		}
        		System.out.println("Decrypted Value : [" + encryptedPass + "]");
        		System.exit(0);
    		}
    	}
    }
    ...
    _initAWSFromConfigMap(mapConfig);
    ...
    private static void _initAWSFromConfigMap(Map<String, Object> mapConfig) {
    	// check aws_smtp_user
      if(mapConfig.containsKey("aws_smtp_user") && mapConfig.get("aws_smtp_user")!=null) {
        try {
          String aws_smtp_user = mapConfig.get("aws_smtp_user").toString().trim();
          if(!aws_smtp_user.isEmpty()) {
            AppModel.AWS_SMTP_USER = aws_smtp_user;
          }
        } catch(Exception e) {}
      }

    	// check aws_smtp_raw_pass
      if(mapConfig.containsKey("aws_smtp_raw_pass") && mapConfig.get("aws_smtp_raw_pass")!=null) {
        try {
          String aws_smtp_raw_pass = mapConfig.get("aws_smtp_raw_pass").toString().trim();
          if(!aws_smtp_raw_pass.isEmpty()) {
            AppModel.AWS_SMTP_PASS = aws_smtp_raw_pass;
          }
        } catch(Exception e) {
          e.printStackTrace();
        }
    	...
    	// check aws_smtp_encrypted_pass
      if(mapConfig.containsKey("aws_smtp_encrypted_pass") && mapConfig.get("aws_smtp_encrypted_pass")!=null) {
        try {
          String aws_smtp_encrypted_pass = mapConfig.get("aws_smtp_encrypted_pass").toString().trim();
          if(!aws_smtp_encrypted_pass.isEmpty()) {
            String pass = SimpleProtector.decrypt(aws_smtp_encrypted_pass);
            AppModel.AWS_SMTP_PASS = pass;
          }
        } catch(Exception e) {
          e.printStackTrace();
        }
      ...
      // check aws_smtp_host
      if(mapConfig.containsKey("aws_smtp_host") && mapConfig.get("aws_smtp_host")!=null) {
        try {
          String aws_smtp_host = mapConfig.get("aws_smtp_host").toString().trim();
          if(!aws_smtp_host.isEmpty()) {
            AppModel.AWS_SMTP_HOST = aws_smtp_host;
          }
        } catch(Exception e) {}
      }
    }
